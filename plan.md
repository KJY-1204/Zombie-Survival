...
## M14 좀비 유휴 배회 (2026-09-22)

- 좀비가 플레이어를 보거나 소리를 조사하지 않을 때 NavMesh 위의 가까운 임의 지점으로 이동하게 한다.
- 배회 지점 도착 뒤에는 잠시 멈추고, 추적·조사는 즉시 배회를 중단하고 기존 우선순위를 유지한다.
- 배회 상태도 기존 `HasTarget` 이동 전환에 전달해, 위치 이동과 걷기 모션을 일치시킨다.
- World Play Mode에서 스폰된 좀비의 위치 변화와 콘솔 오류를 확인한다.

## M15 게임플레이 HUD 스타일 정비 (2026-09-23)

### 목표

- 참고 이미지처럼 게임플레이 정보를 화면 가장자리에 정돈한 생존 슈터 HUD로 재구성한다.
- 기존 체력·무기·상호작용·건설·오토바이 HUD의 런타임 데이터 연결을 유지한다.

### 완료 조건

- 상단에는 실제 플레이어 방향과 생존 구역 표식, 우측에는 현재 조작 키와 안내를 담는 컴팩트한 정보 패널이 표시된다.
- 좌하단 플레이어 상태, 우하단 무기/탄약, 중앙 조준점, 중앙 하단 상호작용 안내가 겹치지 않고 표시된다.
- 실제 구현되지 않은 파티·퀘스트·허기/갈증 시스템은 정적 수치나 가짜 진행도로 추가하지 않는다.
- `HUD Canvas.prefab`의 기존 UI 스크립트 참조가 모두 유효하며, Unity 재컴파일과 Play Mode 콘솔 검증이 통과한다.

### 구현 순서

1. 현재 HUD 프리팹과 연결된 UI 스크립트·게임 데이터의 경계를 확인한다.
2. 재사용 가능한 HUD 생성 도구로 기존 HUD를 새 레이아웃으로 만들고 기존 참조를 다시 연결한다.
3. Unity에서 프리팹을 검증하고 World 씬 Play Mode에서 렌더링과 콘솔을 확인한다.
