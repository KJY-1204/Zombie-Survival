...
- 기준 커밋은 `ebe5f24 10슬롯 저장 화면과 주기 자동저장 추가`이며 `main`과 `origin/main`은 동기화되어 있다. 원격은 `https://github.com/KJY-1204/Zombie-Survival.git`이다.
...
- `ebe5f24 10슬롯 저장 화면과 주기 자동저장 추가`
- `2227583 씬 상태 수집과 복원을 담당하는 SaveManager 추가`
- `0e1fc5f 저장 레지스트리와 저장 DTO, 원자적 슬롯 파일 입출력 추가`
- `95da256 시드 월드에 오토바이와 플레이어 주변 유지형 좀비 스폰을 붙임`
- `b2ae181 시드 청크에 자원과 상자를 결정론적으로 배치하고 채집/루팅 상태를 분리`
- `53e236e M7 저장 시스템 계획과 체크리스트 작성`
