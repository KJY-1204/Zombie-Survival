...
## 다음 세션 시작점 (2026-09-20 기준)

**Claude와 Codex 모두 여기부터 읽으세요.**

- 마지막 커밋은 `f2e9436`이고 `main`과 `origin/main`은 동기화되어 있습니다. 작업 트리는 깨끗합니다(사용자 소유의 `ProjectSettings` 변경 제외).
- Unity는 정지 상태이고 열린 씬은 `Assets/Scenes/World.unity`입니다. 저장돼 있습니다.
- M0~M7과 M8-UI(팰월드풍 UI 리뉴얼)까지 끝났습니다. **코드로 검증할 수 있는 항목은 전부 검증했습니다.**

### 바로 할 일 후보 (권장 순서)

1. **사용자 수동 플레이 확인.** 자동 검증이 불가능한 항목이 `checklist.md`에 13개 남아 있습니다(입력 합성 불가). 키 바인딩, ADS 조작감, 주행감, 채집 조작감, 도로 폭 판단 등입니다. 이걸 먼저 받고 우선순위를 정하는 편이 좋습니다.
2. **게임 시작 흐름.** 지금은 저장 슬롯 화면에서만 불러올 수 있습니다. 타이틀/이어하기 흐름이 없습니다.
3. **루팅 테이블.** 모든 상자가 같은 내용물(탄약 상자 1, 붕대 1, 고철 2)을 줍니다.
4. **POI 실내 루팅.** 자원/상자는 바닥 타일 위에만 놓입니다. 학교 실내에는 전리품이 없습니다.
5. **최적화.** 런타임 NavMesh 베이크 596ms가 가장 비쌉니다. 메시 Read/Write를 켠 모델 132개의 메모리도 함께 볼 것.

### 알려진 미해결 (작게, 기능 영향 있음)

- `LivingEntity.RestoreHealth`에 상한이 없어 붕대를 쓰면 체력이 최대치를 넘습니다(60 -> 110 확인). 교재 코드 그대로라 손대지 않았습니다. 한 줄이면 막힙니다.
- `FindObjectOfType` 폐지 경고가 여러 파일에 남아 있습니다. 기능 영향 없음.
- 라이더의 다리가 약간 길게 내려오는 리타게팅 아티팩트가 남아 있습니다.

### 작업 규칙 요약 (Codex에게도 동일)

- UI는 **반드시 `Assets/Prefabs/HUD Canvas.prefab` 안에서** 고칩니다. 씬의 HUD Canvas는 프리팹 인스턴스라 씬에서 자식을 옮기면 조용히 실패합니다.
- 비사소한 작업 전에 `plan.md` / `checklist.md` / `context-notes.md`를 읽고, 끝나면 같은 파일에 결과를 남깁니다.
- 검증은 `console_status`의 `groundTruth.consoleErrors`로 확인합니다. eval 결과만 보면 예외를 놓칩니다.
- 한 논리 단위마다 커밋하고 바로 push 합니다.
