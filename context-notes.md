...
### M11 진행 - 장착 자세 보정 (2026-09-21)

- 사용자가 활과 근접무기를 들었을 때 방향과 손잡이가 이상하다고 관찰했다. 추측으로 좌표를 바꾸지 않고, 기존 소총 프리팹과 World Play Mode의 실제 양손 IK 결과를 기준으로 활·한손검 래퍼 프리팹만 보정한다.
- 검사 결과 활은 양손 그립이 중심점에 겹쳤고, 한손검도 양손이 같은 손잡이를 사용했다. 소총 자세 재사용이 원인이므로, Archer와 Swordsman 팩의 휴머노이드 Idle·Attack1 클립을 상체 레이어로 재생해 활·검 전용 자세를 만든다. 외부 컨트롤러는 Survivalist와 파라미터가 달라 직접 교체하지 않는다.

### M11 완료 - 장착 자세 보정 (2026-09-21)

- `WeaponAnimationDriver`가 기존 Survivalist 컨트롤러를 하체 기반으로 유지하고, Archer·Swordsman의 Idle·Attack1을 상체 레이어로만 재생한다. 활·검은 총기 컨트롤러나 총기 그립 IK를 사용하지 않는다.
...
### M12 완료 - 진행 현황·에셋 인벤토리 (2026-09-21)

- 현재 에셋 점검의 기준은 `Assets` 아래의 `.meta`를 제외한 실제 파일이다. Unity `.meta`는 같은 에셋의 부수 메타데이터이므로 파일 이름 목록을 중복시키지 않는다.
- `docs/asset-inventory.tsv`에는 경로와 바이트 크기를 기록한다. 경로 누락뿐 아니라 같은 경로의 의도하지 않은 파일 교체도 크기 변경으로 감지한다.
- `Tools/Verify-AssetInventory.ps1`은 기본 실행에서 누락·추가·크기 변경을 실패로 반환한다. 새 에셋을 의도적으로 추가하거나 제거했을 때만 `-Update`로 목록을 갱신하고 관련 변경과 함께 커밋한다.
- 다음 구현 우선순위는 활·석궁·검의 피해 시점 애니메이션 연동, 맨손 피해 판정, 무기별 좀비 대상 Play Mode 검증이다. 새 게임·수동 저장·자동 저장 복원 검증은 사용자가 이전에 보류한 항목으로 남는다.

### M11 착수 - 전투 애니메이션 명중 시점 연동 (2026-09-22)

- 활 `Archer@Attack1`의 `Hit` 이벤트는 0.40163934초, 석궁 `Crossbow@Attack1`은 0.26956522초에 있고, 한손검 `Swordsman@Attack1`은 0.31670824초와 0.5386534초에 있다.
- 한손검 클립의 두 번째 `Hit`가 한 번의 입력으로 피해를 두 번 만들지 않게, 입력에서 예약한 공격 하나만 첫 이벤트에서 해결하고 뒤 이벤트는 무시한다.
- 외부 클립이나 이벤트는 수정하지 않는다. `WeaponAnimationDriver`가 기존 `Hit()` 수신을 현재 장착 무기로 전달하고, 무기별 기존 발사·피해 계산은 각 무기에 남긴다.

### M11 완료 - 전투 애니메이션 명중 시점 연동 (2026-09-22)

- `BeginAttack()`은 공격 가능한 활·석궁·한손검에만 예약 상태를 만들고, `ResolveAnimationHit()`은 예약이 있을 때만 기존 `Fire()`를 호출한다. 총기와 맨손은 기존 즉시 경로를 유지한다.
- `WeaponAnimationDriver.Hit()`은 이벤트 구독자로 전달한다. 한손검의 두 번째 `Hit`는 첫 이벤트가 예약을 소진한 뒤라 무시된다.
- 공격 클립이 끝나기 전 다음 예약을 막는다. 명중 후에도 클립은 잠시 재생되므로, 이 가드가 없으면 지속 입력이 이벤트를 다시 지나지 않는 기존 클립에 다음 공격을 예약할 수 있다.
- World Play Mode에서 활과 석궁은 각각 장전된 탄창 1발이 공격 클립 뒤 0발이 됐고, 한손검은 체력 100인 임시 대상 하나를 45로만 낮췄다. 한손검 클립의 두 `Hit`가 중복 피해를 만들지 않음을 확인했다.
- 강제 재컴파일은 성공했고 콘솔 오류는 0개다. 외부 에셋 경고 4개만 남아 있다.

### M11 착수 - 맨손 공격 판정 (2026-09-22)

- 맨손은 장착 무기가 없으므로 `PlayerShooter`가 명중 이벤트의 예약과 실행을 맡는다. 피해 수치는 무기와 같은 `MeleeWeaponData` 정적 에셋으로 분리한다.
- 첫 단계는 콤보·넉백·무기 없는 장비 슬롯을 추가하지 않고, 전방 부채꼴의 가장 가까운 생명체 하나를 한 번 타격하는 범위로 제한한다.

### M11 완료 - 맨손 공격 판정 (2026-09-22)

- `Unarmed Attack` 에셋은 피해 20, 사거리 1.5, 각도 100도, 재사용 대기시간 0.7초, 소음 반경 5로 설정했고 `Player Character` 프리팹에 연결했다.
- World Play Mode에서 무기를 해제한 플레이어의 애니메이션 `Hit` 이벤트 뒤 임시 대상 체력은 100에서 80으로 한 번만 감소했다. 첫 예약은 성공하고 연속 예약은 차단됐으며, 명중 직후 재예약도 차단됐다.
- 강제 재컴파일은 성공했고, Play Mode 종료 뒤 콘솔 오류와 경고는 모두 0개다.

### M12 착수 - 실제 좀비 대상 전투 검증 (2026-09-22)

- 전 단계의 임시 `LivingEntity` 검증은 피해 계약과 애니메이션 시점은 보였지만, 실제 `Zombie` 컴포넌트·콜라이더 조합까지 보장하지 않는다. World Play Mode에서 실재 좀비 하나를 통제된 전방 위치에 두고 네 공격 경로를 확인한다.
- 이 단계는 검증 전용이다. 동작 실패가 없다면 코드·프리팹·에셋을 새로 만들지 않는다.

### M12 완료 - 실제 좀비 대상 전투 검증 (2026-09-22)

- World 스포너가 현재 NavMesh 위 유효한 생성 지점을 찾지 못해, 스포너에 연결된 실제 `Zombie` 프리팹과 `ZombieData`로 런타임 인스턴스 하나를 만들고 AI만 잠시 비활성화했다. 이 객체와 테스트 설정은 Play Mode 종료 때 사라진다.
- 실제 `Zombie` 체력은 한손검 100에서 45, 맨손 100에서 80, 활 100에서 58, 석궁 100에서 35로 감소했다. 활·석궁은 전용 애니메이션의 `Hit` 이벤트에서 투사체를 만든 뒤 실제 콜라이더에 명중했다.
- Pipeline이 백그라운드에서 게임 시간을 멈추는 환경에서는 `Application.runInBackground`를 테스트 런타임에만 켜고, 쿨다운 대기는 테스트 인스턴스의 마지막 공격 시각을 조정해 준비했다. 플레이어 빌드 동작이나 에셋에는 영향을 주지 않는다.
- Play Mode 종료 뒤 `consoleErrors: 0`, `consoleWarnings: 0`을 확인했다.

### M13 착수 - 초원 세트드레싱 밀도 (2026-09-22)

- 현재 초원 장식은 `ChunkLibrary.plainPropMin=0`, `plainPropMax=3`이라 빈 청크가 자주 생긴다. 건물 수를 바로 늘리면 50m 청크 안의 충돌·접근성을 다시 설계해야 하므로, 먼저 충돌 위험이 낮은 기존 나무 장식만 6~10개로 늘린다.
- 자원·루팅·좀비 수는 게임 밸런스에 영향을 주므로 이번 시각 밀도 변경에서 제외한다.

### M13 완료 - 초원 세트드레싱 밀도 (2026-09-22)

- `Chunk Library`의 기존 나무 프리팹 두 종류는 유지하고, 초원 장식 수만 청크당 0~3개에서 6~10개로 올렸다. 로드 반경의 초원 14개는 바닥·자원·상자를 포함해 각 8~16개 자식을 가졌으므로, 장식물 최소 6개가 실제로 배치됐다.
- World Play Mode의 유일한 신규 오류는 Play Mode 전환 직후 Pipeline 메인 스레드 요청이 5초를 넘긴 도구 통신 오류였다. 게임 코드·월드 생성 오류는 확인되지 않았고, Play Mode 종료 뒤 해당 오류가 아닌 콘솔 경고는 0개다.
- 도시 건물 수, 자원·상자·좀비 밀도는 밸런스와 접근성 검토가 필요한 별도 단계로 남긴다.
- 콘솔을 비운 뒤 World Play Mode를 다시 실행·종료해 `compilationFailed: false`, `consoleErrors: 0`, `consoleWarnings: 0`을 재확인했다.

### M14 착수 - 좀비 유휴 배회 (2026-09-22)

- 기존 `Zombie.UpdatePath`는 목표와 조사 지점이 모두 없을 때 에이전트를 멈춘다. 이 분기만 배회로 대체해, 시야 추적과 소리 조사 경로를 바꾸지 않는다.
- 배회는 현재 위치 기준 8m 안에서 `NavMesh.SamplePosition`으로 유효한 지점을 찾고, 도착 후 2~4초 쉰다. 새 데이터 에셋이나 전역 상태는 추가하지 않는다.

### M14 완료 - 좀비 유휴 배회 (2026-09-22)

- 유휴 분기는 NavMesh 위 3~8m 지점을 골라 이동하고, 도착하면 2~4초 쉰다. 플레이어를 보거나 소리를 조사하는 즉시 배회 상태를 해제해 기존 추적·조사 목적지가 우선한다.
- World Play Mode에서 스포너는 8마리를 정상 생성했고, 플레이어 탐지 거리 밖의 한 좀비는 5초 사이 `(615.23, -0.96, 498.70)`에서 `(617.17, -0.97, 497.45)`로 약 2.3m 이동했다.
- 강제 재컴파일은 성공했고 Play Mode 종료 뒤 `compilationFailed: false`, `consoleErrors: 0`, `consoleWarnings: 0`을 확인했다.

### M14 보완 - 배회 이동 모션 (2026-09-22)

- `Zombie Character` 컨트롤러는 `HasTarget`이 true일 때 Idle에서 Move로 전환하고 `Zombie Run` 클립을 재생한다. 이전 배회 구현은 이 불리언에 `isPatrolling`을 포함하지 않아 실제 위치가 이동해도 Idle 모션이 남았다.
