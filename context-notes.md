...
## 2026-09-18 - 몸 숨김 1인칭 폐기, 몸 전체가 보이는 근접 시점으로 전환 + 자기 피격 버그 수정 (Claude)

사용자 요청: "일단 플레이어 케릭터에 모션들을 적용해봐... 사람이 투명상태가 되는걸 해제하고 무기를 제대로 잡고있는 모션으로 만들어봐 시점 카메라가 가려지면 카메라 좌표를 움직여서 맞춰보면 되니깐". 이후 실사용 중 "총이 플레이어 머리뒤로 가서 총쏘면 플레이어가 죽잖아" 라는 추가 버그 리포트.

### 배경 - 왜 몸을 숨겼었는지, 왜 다시 보이게 했는지

- 이전 세션들에서는 "진짜 눈 위치에 카메라를 두는 순수 FPS" 방식을 택했고, 그러면 카메라가 필연적으로 캐릭터 자신의 머리/몸 메시 내부에 위치하게 되어 몸통 렌더러를 꺼야 했다(그 결과 전용 `FPS_HANDS` 뷰모델만 남기고 나머지를 숨김).
- 사용자는 이 "몸이 투명해지는" 상태를 원하지 않았고, 대신 카메라 좌표를 조정해서라도 몸 전체(특히 총을 쥔 모습)가 보이게 해달라고 요청함 - 즉 순수 눈높이 FPS 대신 **몸 뒤쪽 어깨너머에 가까운 근접 3인칭 카메라**로 방향을 전환.

### 변경 - `Assets/Scripts/Camera/CameraRigController.cs`

- `bodyRoot` 하위 모든 `SkinnedMeshRenderer`를 더 이상 숨기지 않고 전부 `enabled = true`로 설정 (기존에는 `firstPersonVisiblePartNames`와 이름이 일치하는 것만 켜고 나머지를 껐음). 이제 사용하지 않게 된 `firstPersonVisiblePartNames` 필드는 제거.
- `firstPersonHands`(FPS_HANDS 전용 뷰모델)는 몸 전체가 보이는 상황에서 팔이 두 겹으로 겹쳐 보이지 않도록 `SetActive(false)`로 명시적으로 꺼둠(코드와 씬 직렬화 값 모두 정리).
- `playerShooter.useFirstPersonMount`를 `true`->`false`로 변경 - 카메라 전용 마운트(눈앞 고정 위치) 대신 **기존 3인칭용 팔꿈치 IK 기반 총 배치**를 사용하도록 되돌림. 이렇게 하면 실제 조준 자세 애니메이션(`HumanM@AssaultRifle_Aim01`)과 손-총 IK 정렬이 그대로 "총을 제대로 쥔" 모습을 만들어준다 - 별도의 그립 자세를 새로 만들 필요가 없었음.

### 카메라 위치 조정 (씬에 저장, `FirstPerson Cam`의 로컬 좌표)

- 기존 눈높이 값 `(0, 1.78, 0.15)`(카메라가 머리 메시 내부에 위치, Z=0.15가 머리 앞쪽 범위 -0.25~0.17 안에 들어가 있어 카메라가 얼굴 속에 파묻혀 있었음 - 그래서 몸을 지우지 않으면 얼굴 뒤쪽(backface)만 보여 아무것도 안 보였음)를 여러 단계로 조정:
  - 1차 시도 `(0, 2.0, -0.6)`: 머리 위/뒤로 이동 - 몸은 보였지만 총이 카메라 근처(옛 눈높이 마운트 기준)에 남아있어 팔/어깨와 완전히 분리되어 보임.
  - `useFirstPersonMount=false`로 전환한 뒤 `(0, 1.95, -0.5)`: 총이 팔꿈치 IK를 따라가면서 어깨 쪽에 훨씬 자연스럽게 위치.
  - 최종 `(0, 2.2, -1.1)`: 좀 더 뒤로/위로 당겨서 머리(모자)와 배낭끈, 총 전체가 화면에 들어오는 어깨너머 구도로 확정.
- 이 값들은 캐릭터 스케일(캡슐 height 1.9)과 현재 애니메이션 포즈 기준으로 눈대중 조정한 것이라, 실제 플레이 시 사람이 보면서 좀 더 미세 조정하고 싶을 수 있음 - `FirstPerson Cam`의 `Transform.localPosition` 하나만 바꾸면 됨.

### 버그 발견 및 수정 - 조준 레이가 자기 몸을 맞춰 자살하는 문제

- 카메라를 캐릭터 몸 뒤(Z=-1.1)로 옮기면서, `Gun.Shot()`이 쏘는 "화면 중앙 기준 조준 레이"(지난 세션에 추가한 크로스헤어 발사 로직)가 카메라 위치(몸 뒤)에서 카메라 정면 방향으로 나가기 때문에, **레이가 캐릭터 자신의 `CapsuleCollider`(이동/물리용, Player Character 루트에 있음)를 먼저 통과하며 맞춰버리는** 문제가 발생함. `Gun.Shot()`은 `hit.collider.GetComponent<IDamageable>()`을 호출하고 `PlayerHealth`가 바로 그 `IDamageable`이라, 총을 쏘면 즉시 자기 자신에게 데미지를 주는 버그로 이어짐(사용자가 실제 플레이 중 발견).
- 수정 (`Assets/Scripts/Gun.cs`, `Assets/Scripts/PlayerShooter.cs`):
  - `Gun`에 `ownerRoot` 필드와 `public void SetOwner(Transform owner)` 메서드 추가.
  - `PlayerShooter.Start()`에서 `gun.SetOwner(transform)` 호출 - 씬에서 수동으로 연결할 필요 없이 코드로 자동 등록.
  - `Gun.Shot()`을 단일 `Physics.Raycast` 대신 `Physics.RaycastAll` + 자기 자신 제외 후 가장 가까운 충돌을 고르는 `TryGetClosestHit()` 헬퍼로 교체. `hit.transform.IsChildOf(ownerRoot)`로 자기 자신(과 그 자식)의 콜라이더를 걸러냄.
  - 카메라가 몸 앞이든 뒤든 상관없이 항상 안전하게 동작하도록 일반화된 수정이라, 나중에 카메라 위치를 또 조정해도 이 버그가 재발하지 않음.

### 검증

- 컴파일 오류 0건.
- Play 모드에서 몸 전체(머리/모자/배낭/부츠 등) 렌더러가 전부 `enabled=true`로 보임을 확인, `FPS_HANDS`는 `activeSelf=false`로 꺼짐 확인.
- `source: "screen"` 캡처로 어깨너머 구도(머리/모자/배낭끈 + 오른쪽에 총열 일부)가 자연스럽게 보이는 것을 시각적으로 확인. 몸이 더 이상 투명하지 않음.
- 자기 피격 버그 수정 검증: `shooter.gun.Fire()`를 5번 연속 호출해도 `PlayerHealth.health`가 100에서 전혀 줄지 않고 `dead=false` 유지됨을 확인 (수정 전이었다면 즉시 죽었을 상황).
- `PlayerTestRig.prefab` 재생성 완료 (카메라 위치 변경 반영).

### 남은 위험 / 다음 확인 사항

- 조명 문제로 추정되는 현상 발견: 캡처한 스크린샷에서 캐릭터가 전반적으로 어두운 실루엣으로 보임(역광 추정). 이번 요청 범위(모션/카메라/자기피격) 밖이라 손대지 않았음 - 나중에 조명/그림자 설정을 점검할 필요가 있음.
- 최종 카메라 좌표 `(0, 2.2, -1.1)`는 초기 추정치이므로, 실제 사람이 보면서 각도/거리를 더 다듬을 필요가 있음. 마우스로 위아래를 볼 때(`FirstPersonLook` 피치) 이 어깨너머 구도가 여전히 자연스러운지도 확인 필요.
- 총 렌더링은 여전히 `FirstPersonWeaponLayer` 전용 카메라(근평면 0.01)를 거치는데, 이제 총이 팔꿈치 IK로 몸 가까이 위치하므로 몸(기본 카메라, 근평면 0.1)과의 렌더링 순서/오클루전이 항상 정확한지는 아직 시각적으로 세밀히 검증하지 않음 - 나중에 손이 총 일부를 가려야 하는 각도에서 확인해볼 것.
