...
- [x] `compilationFailed: false`, `consoleErrors: 0`을 확인한다.
- [x] 커밋하고 원격 `main`에 push한다. (0e1fc5f)
...
- [x] `TabView` 공통 뼈대를 만들고 인벤토리/장비/상태를 탭으로 옮긴다.
- [x] `CharacterScreenUI`가 여닫기/커서/단축키/탭 전환을 담당한다.
- [x] HUD Canvas **프리팹을** 고쳐 두 씬이 같은 UI를 쓰게 한다 (씬에서는 프리팹 인스턴스 자식을 옮길 수 없다).
- [x] M7에서 씬에만 넣었던 저장 화면도 프리팹으로 옮기고 중복 오버라이드를 정리한다.
...
- [x] 아이콘 + 이름 + 개수 + 무게로 된 줄 프리팹을 만든다 (`Assets/Prefabs/UI/Item Row.prefab`).
- [x] 줄을 선택하면 강조되고 오른쪽 상세 패인에 설명과 동작 버튼이 나온다.
- [x] 무게를 막대 그래프로 보여주고 과적이면 색이 바뀐다 (초록 -> 노랑 -> 빨강).
...
- [x] 장비 탭을 슬롯 6줄 + 장착 후보 목록 두 열로 바꾼다 (줄을 누르면 장착/해제).
- [x] 상태 탭을 체력 막대 + 능력치/장착 카드로 바꾼다.
- [x] 보관 상자 화면을 같은 테마로 바꾼다 (양쪽 목록 + 무게 막대, 줄 클릭으로 이동).
- [x] 건설 메뉴를 같은 테마로 바꾼다 (아이콘 추가, 재료 부족은 붉게).
...
- [x] 체력바를 좌하단으로 옮기고 팔월드풍으로 바꾼다 (슬라이더 유지 + 수치 겹침).
...
- [x] 공통 장착 무기 계약과 투사체·근접 무기 동작을 추가한다.
- [x] PlayerShooter와 HUD를 공통 계약으로 전환하고 기존 Gun 동작을 유지한다.
- [x] SMG·활·석궁·한손검과 화살·볼트 아이템을 실제 에셋으로 만든다.
- [x] Save Registry와 POI 루팅 테이블에 새 무기·탄약을 연결한다.
- [x] Unity 컴파일과 무기별 기본 공격 동작을 검증한다.
- [x] 커밋하고 원격 main에 push한다.

## 2026-09-21 - M11 장착 자세 보정

- [x] 활·한손검 프리팹과 기존 소총의 축·손잡이 변환을 비교한다.
- [x] World Play Mode Game View에서 각 무기를 장착해 실제 자세를 캡처한다.
- [x] Archer·Swordsman의 휴머노이드 Idle·Attack1 클립을 상체 전용 레이어로 연결한다.
- [x] 활·한손검의 모델 회전·위치와 양손 그립 목표를 최소 보정한다.
...
## 2026-09-22 - M11 전투 애니메이션 명중 시점 연동

- [x] 활·석궁·한손검 공격 클립의 `Hit` 이벤트 시점을 확인한다.
- [x] 공통 장착 무기에 공격 예약과 애니메이션 명중 해결 경로를 추가한다.
- [x] 활·석궁·한손검의 피해·투사체 생성을 명중 이벤트로 옮긴다.
- [x] 총기·맨손의 기존 즉시 동작이 유지되는지 확인한다.
- [x] Unity 컴파일과 World Play Mode에서 무기별 명중 결과 및 콘솔 오류를 검증한다.
- [ ] 원격 main에 push한다. (로컬 커밋 `1abf4cc` 생성 완료. 원격 전송 승인 대기)

## 2026-09-22 - M11 맨손 공격 판정

- [x] 맨손 모션의 `Hit` 이벤트와 기존 장착 무기 판정 경로를 확인한다.
- [x] 맨손 공격 정적 데이터와 플레이어 참조를 만든다.
- [x] `Hit` 이벤트에서 전방 대상 하나에 피해를 적용한다.
- [x] Unity 컴파일과 World Play Mode에서 명중·쿨다운·콘솔 오류를 검증한다.
- [ ] 커밋하고 원격 main에 push한다.

## 2026-09-22 - M12 실제 좀비 대상 전투 검증

- [x] 활·석궁·한손검·맨손의 공격 경로와 `Zombie` 피해 계약을 확인한다.
- [x] World Play Mode에서 실제 좀비 대상으로 무기별 피해 결과를 확인한다.
- [x] Unity 콘솔 오류를 확인한다.
- [ ] 문서화한 검증 기록을 커밋하고 원격 main에 push한다.

## 2026-09-22 - M13 초원 세트드레싱 밀도

- [x] 월드 청크의 현재 장식물·건물 배치 규칙과 초원 밀도 값을 확인한다.
- [x] 초원 장식물을 청크당 6~10개로 조정한다.
- [x] World Play Mode에서 로드된 청크의 장식물 수와 콘솔 오류를 확인한다.
- [ ] 변경을 커밋하고 원격 main에 push한다.

## 2026-09-22 - M14 좀비 유휴 배회

- [x] 좀비의 유휴·추적·소리 조사 경로와 NavMesh 사용 상태를 확인한다.
- [x] 유휴 상태에서 가까운 NavMesh 지점을 오가는 배회를 추가한다.
- [x] 배회 상태에서 기존 걷기 애니메이션 전환이 재생되는지 확인한다.
- [x] World Play Mode에서 스폰된 좀비의 위치 변화와 콘솔 오류를 확인한다.
- [ ] 변경을 커밋하고 원격 main에 push한다.
...
- [x] 검증된 변경을 커밋하고 원격 main에 push한다. (`adbea74`)
