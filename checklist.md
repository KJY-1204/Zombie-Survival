...
- [ ] `UiTheme`에 색/여백/글자 크기를 모은다.
- [ ] 에디터 전용 아이콘 생성기를 만든다 (프리팹을 카메라로 찍어 PNG로 저장).
- [ ] 13종 아이템의 `icon`을 채운다 (모델 없는 방어구 3종은 색 타일).
- [ ] 생성한 스프라이트의 임포트 설정(Sprite, 투명 배경)을 확인한다.
- [ ] `compilationFailed: false`, `consoleErrors: 0`을 확인한다.
- [ ] 커밋하고 원격 `main`에 push한다.

### 2단계. 통합 창과 탭

- [ ] `TabView` 공통 뼈대를 만들고 인벤토리/장비/상태를 탭으로 옮긴다.
- [ ] `CharacterScreenUI`가 여닫기/커서/단축키/탭 전환을 담당한다.
- [ ] `I`/`O`/`K`가 같은 창을 각 탭으로 연다.
- [ ] 같은 키를 다시 누르면 닫힌다.
- [ ] 기존 동작(사용/버리기/장착/해제)이 그대로인지 확인한다.
- [ ] `CLAUDE.md` §11.1과 `GAME_DESIGN.md`의 "화면 분리" 규칙을 갱신한다.
- [ ] `compilationFailed: false`, `consoleErrors: 0`을 확인한다.
- [ ] 커밋하고 원격 `main`에 push한다.
...
- [ ] 장비 탭을 슬롯 카드 + 장착 후보 목록으로 바꾼다.
- [ ] 상태 탭을 수치 카드로 바꾼다.
- [ ] 보관 상자 화면을 같은 테마로 바꾼다.
- [ ] 건설 메뉴를 같은 테마로 바꾼다 (재료 부족은 색으로 구분).
- [ ] 저장 슬롯 화면을 같은 테마로 바꾼다.
- [ ] `compilationFailed: false`, `consoleErrors: 0`을 확인한다.
- [ ] 커밋하고 원격 `main`에 push한다.

### 5단계. HUD

- [ ] 체력바를 좌하단으로 옮기고 팰월드풍으로 바꾼다.
- [ ] 우하단에 장착 무기와 탄약을 표시한다.
- [ ] 상호작용 안내와 건설 안내를 키 배지 형태로 바꾼다.
- [ ] 차량 상태(연료/내구도)를 같은 테마의 막대로 바꾼다.
- [ ] Play Mode에서 값이 실제로 바뀌는지, 화면을 벗어나지 않는지 검증한다.
- [ ] `compilationFailed: false`, `consoleErrors: 0`을 확인한다.
- [ ] 커밋하고 원격 `main`에 push한다.
